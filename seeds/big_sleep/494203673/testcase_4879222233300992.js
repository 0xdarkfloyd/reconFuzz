// Bundled wasm-module-builder.js. Actual testcase at the bottom.
load = function() {};
if (typeof d8 !== 'undefined') {
  if (d8.file) d8.file.execute = function() {};
}
...
let sig_v_v_index = builder.addType(kSig_v_v);
let cont_index = builder.addCont(sig_v_v_index);
let tag_index = builder.addTag(makeSig([wasmRefNullType(cont_index)], []));

let dummy = builder.addFunction("dummy", sig_v_v_index).addBody([]).exportFunc();
builder.addFunction("main", sig_v_v_index)
  .addBody([
...
try {
  ...
  let cont_obj = values[0];
  print("Leaked continuation: " + cont_obj);
  %DebugPrint(cont_obj);
}
